...
pub const SPACE_RANGES: &[u8; 16] =
    b"\x00\x08\x0e\x1f!\xff\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00";
pub const NOT_TOKEN_RANGES: &[u8; 16] = b"=={{}}\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00";
pub const STRING_LITTERAL_CONTENT_RANGES: &[u8; 16] =
    b"\"\"\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00";

pub const IDENTIFIER_RANGES: &[u8; 16] = b"\x00\x2d\x2f\x2f;@[^``{\xff\x00\x00\x00\x00";
...
pub fn take_simd_identifier<'a>(input: &'a str) -> Res<&'a str, &'a str> {
    take_while_simd::<'a, _>(is_identifier_char, IDENTIFIER_RANGES)(input)
}

#[inline(always)]
pub fn take_simd_string_literal<'a>(input: &'a str) -> Res<&'a str, &'a str> {
    take_while_simd::<'a, _>(is_string_litteral_contents, STRING_LITTERAL_CONTENT_RANGES)(input)
}

#[inline(always)]
pub fn take_simd_not_token<'a>(input: &'a str) -> Res<&'a str, &'a str> {
    take_while_simd::<'a, _>(move |character| !is_token(character), NOT_TOKEN_RANGES)(input)
}

#[inline(always)]
pub fn take_simd_space<'a>(input: &'a str) -> Res<&'a str, &'a str> {
    take_while_simd::<'a, _>(is_space, SPACE_RANGES)(input)
}
#[inline(always)]
fn take_while_simd<'a, Condition>(
    cond: Condition,
...
fn simd_loop16<'a>(string: &'a str, character_ranges: &[u8; CHUNK_SIZE]) -> Res<&'a str, &'a str> {
    ...
    let _character_ranges_length = character_ranges.len() as i32;
    loop {
    ...
    Ok((substring_after, substring_before))
}
...
mod tests {
    ...
    use super::*;
    #[test]
    fn take_while_simd_string_with_leading_whitespace_whitespace_collected_remainder_returned() {
        let text = " \t\n\r|Stop this is a big long string";
        let ranges = SPACE_RANGES;
        let (remainder, parsed) = take_while_simd::<'_, _>(is_space, ranges)(text).unwrap();
        assert_eq!(remainder, "|Stop this is a big long string");
        assert_eq!(parsed, " \t\n\r");
    }

    #[test]
    fn take_while_simd_16_character_string_whitespace_collected_remainder_returned() {
    ...
    #[test]
    fn take_while_simd_16_newlines_1_1_whitespace_collected_remainder_returned() {
        let text = "\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n1";
        let ranges = SPACE_RANGES;
        let (remainder, parsed) = take_while_simd::<'_, _>(is_space, ranges)(text).unwrap();
    ...
    #[test]
    fn take_while_simd_17_newlines_1_1_whitespace_collected_remainder_returned() {
        let text = "\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n1";
        let ranges = SPACE_RANGES;
        let (remainder, parsed) = take_while_simd::<'_, _>(is_space, ranges)(text).unwrap();
        assert_eq!(remainder, "1");
    ...
    #[test]
    fn take_while_simd_string_with_many_leading_whitespace_whitespace_collected_remainder_returned()
    {
    ...
    #[test]
    fn take_while_simd_short_string_whitespace_collected_remainder_returned() {
        let text = "\t\t\ts";
        let ranges = SPACE_RANGES;
        let (remainder, parsed) = take_while_simd::<'_, _>(is_space, ranges)(text).unwrap();
    ...
    #[test]
    fn take_while_simd_all_white_space_whitespace_collected_remainder_returned() {
        let text = " \t\n\r";
        let ranges = SPACE_RANGES;
        let (remainder, parsed) = take_while_simd::<'_, _>(is_space, ranges)(text).unwrap();
        assert_eq!(remainder, "");
